...
public:
  ...
  time_duration GetTimeout() { return _simulator->GetNetworkingTimeout(); }
  ...
  World LoadWorld(std::string map_name, bool reset_settings = true,
    ...
    return World{_simulator->LoadEpisode(std::move(map_name), reset_settings,
                                         map_layers)};
  }

  /// Return (and load) a new world (map) only when the requested map is
  /// different from the current one

  void
  LoadWorldIfDifferent(std::string map_name, bool reset_settings = true,
                       rpc::MapLayer map_layers = rpc::MapLayer::All) const {
    ...
    if (!(map_name_without_prefix == current_map_name) &&
        !(map_name_with_prefix == current_map_name)) {
      World World{_simulator->LoadEpisode(std::move(map_name), reset_settings,
                                          map_layers)};
    } else {
    }
  }
  ...
  World GenerateOpenDriveWorld(std::string opendrive,
    ...
    return World{_simulator->LoadOpenDriveEpisode(std::move(opendrive), params,
                                                  reset_settings)};
  }

  /// Return an instance of the world currently active in the simulator.
  World GetWorld() const { return World{_simulator->GetCurrentEpisode()}; }

  /// Return an instance of the TrafficManager currently active in the
  /// simulator.
  ...
  void StopRecorder(void) { _simulator->StopRecorder(); }
  ...
  std::string ShowRecorderActorsBlocked(std::string name, double min_time,
                                        double min_distance) {
  ...
  void StopReplayer(bool keep_actors) { _simulator->StopReplayer(keep_actors); }
  ...
  void ApplyBatch(std::vector<rpc::Command> commands,
                  bool do_tick_cue = false) const {
    _simulator->ApplyBatch(std::move(commands), do_tick_cue);
  }

  std::vector<rpc::CommandResponse>
  ApplyBatchSync(std::vector<rpc::Command> commands,
                 bool do_tick_cue = false) const {
  ...
  void SetVodafoneAlertImage(int32_t index) {
    _simulator->SetVodafoneAlertImage(index);
  }
...
inline Client::Client(const std::string &host, uint16_t port,
                      size_t worker_threads)
    : _simulator(new detail::Simulator(host, port, worker_threads),
